      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center gap-4 py-16 text-center">
          ...
          <div>
            ...
            <p className="mt-1 max-w-sm text-sm text-muted-foreground">
              Create your first campaign to start finding opportunities and generating outreach
            </p>
          </div>
          <Button render={<Link href="/campaigns/new" />}>
        ...
        <div className="space-y-8">
          {activeCampaigns.length > 0 && (
            <CampaignSection label="Active" count={activeCampaigns.length} campaigns={activeCampaigns} />
          )}
          {draftCampaigns.length > 0 && (
            <CampaignSection label="Drafts" count={draftCampaigns.length} campaigns={draftCampaigns} />
          )}
          {archivedCampaigns.length > 0 && (
            <CampaignSection label="Archived" count={archivedCampaigns.length} campaigns={archivedCampaigns} />
          )}
        </div>
      )}
    </div>
...
function CampaignSection({ label, count, campaigns }: { label: string; count: number; campaigns: Array<{ _id: string; name: string; type: CampaignType; status: CampaignStatus; prospectCount: number; _creationTime: number }> }) {
  return (
                ...
                <CardContent>
                  ...
                  <div className="mt-4 flex items-center justify-between text-sm">
                    <span className="text-muted-foreground">
                      {campaign.prospectCount} prospect{campaign.prospectCount !== 1 ? "s" : ""}
                    </span>
